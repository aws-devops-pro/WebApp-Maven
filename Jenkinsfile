...
try {
  stage('Code from SCM') {
    checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'iamgithub', url: 'https://github.com/iamssp25/WebApp-Maven.git']]])
    sh "ls -ltr"
  }
  stage('Package') {
  sh "cd ${WORKSPACE}"
  ...
  stage('Deploy') {
    ...
    sh "sudo cp ${WORKSPACE}/target/*.war /home/ec2-user/tomcat8/webapps/"
  }
